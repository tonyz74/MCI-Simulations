...
double ais1d_integrate_combined(ais1d_state *st) {
    ...
    double wt_sum = (res_init.mean / res_init.var)
        + (res_fin.mean / res_fin.var);

    double sum_wt = (1.0 / res_init.var)
        + (1.0 / res_fin.var);

    return wt_sum / sum_wt;
}
...
int main(void) {
    ...
    if (mode == TRIALS) {
        ...
        for (int i = 0; i < N_TRIALS; i++) {
            ...
            trials[i] = ais1d_integrate_combined(&ig);
        }
